          ...
          <div className={`flex-1 min-h-0 flex flex-col items-center justify-center md:justify-start md:absolute md:inset-auto md:flex-none gap-2.5 ${widgetCornerClass}`}>
            ...
            {widgetOpen && maybePortal(
              ...
              >
                ...
                {a11yOpen && (
                  <div className={`border-b px-3 py-2.5 space-y-2.5 ${highContrast ? "bg-black border-white/20" : "bg-muted/40 border-border"}`}>
                    <div className="flex items-center justify-between">
                      <span className={`text-2xs font-semibold flex items-center gap-1.5 ${highContrast ? "text-white" : "text-foreground"}`}>
                        <Accessibility className="w-3.5 h-3.5" /> Accesibilidad
                      </span>
                      <button
                        type="button"
                        onClick={() => setA11yOpen(false)}
                        className={`p-1 -m-1 rounded ${highContrast ? "text-white/70 hover:text-white hover:bg-white/10" : "text-muted-foreground/70 hover:text-foreground hover:bg-muted-foreground/10"}`}
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span className={`text-2xs ${highContrast ? "text-white/80" : "text-muted-foreground"}`}>Tamaño de texto</span>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span className={`text-2xs ${highContrast ? "text-white/80" : "text-muted-foreground"}`}>Alto contraste</span>
                      <button
                        type="button"
                        role="switch"
                    ...
                    {ttsSupported && (
                      <p className={`text-3xs leading-snug ${highContrast ? "text-white/60" : "text-muted-foreground/80"}`}>
                ...
                <>
                {/* Mensajes */}
                <div ref={chatScrollRef} className={`flex-1 overflow-y-auto p-3 space-y-2.5 ${highContrast ? "bg-black" : "bg-background"}`}>
                ...
                {enableEscalation && escalState === "hidden" && csatState === "hidden" && (
                  <div className={`border-t px-3 py-1.5 shrink-0 text-center ${highContrast ? "bg-black border-white/20" : "bg-background border-border"}`}>
                    <button
                      type="button"
                      onClick={() => setEscalState("prompt")}
                      className={`text-3xs underline transition-colors ${highContrast ? "text-white/70 hover:text-white" : "text-muted-foreground hover:text-foreground"}`}
                    >
                ...
                {showSuggestions && csatState === "hidden" && escalState === "hidden" && (
                  <div className={`border-t px-2.5 pt-2 pb-1.5 shrink-0 flex flex-wrap gap-1 ${highContrast ? "bg-black border-white/20" : "bg-background border-border"}`}>
                    {suggestions.slice(0, 3).map((s, i) => (
                      <button
                        key={i}
                        onClick={() => handleSend(s)}
                        className={`text-3xs px-2.5 py-1 rounded-full border transition-colors ${highContrast ? "border-white/40 bg-black text-white hover:bg-white/10" : "border-border bg-background hover:bg-muted text-foreground"}`}
                      >
                        {s}
                      </button>
                    ))}
                ...
                {csatState === "hidden" && (
                  <div className={`px-2.5 py-2.5 shrink-0 ${highContrast ? "bg-black" : "bg-background"} ${showSuggestions ? "" : `border-t ${highContrast ? "border-white/20" : "border-border"}`}`}>
                    <div className="flex gap-1.5 items-center">
                      <input
                        className={`flex-1 h-9 rounded-full px-4 text-xs outline-none border transition-colors ${highContrast ? "bg-black border-white text-white placeholder:text-white/50 focus:border-white" : "bg-muted border-transparent placeholder:text-muted-foreground focus:border-primary"}`}
